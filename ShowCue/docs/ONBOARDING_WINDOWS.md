...
**Đường dẫn chuẩn trên máy Windows của team:** `C:\app\dgpcorp` (tạo thư mục `C:\app` nếu chưa có).
...
[ ] Chạy ShowCue.exe OK
[ ] Mở Cursor/VS Code tại C:\app\dgpcorp (thư mục root, không phải ShowCue/)
```
...
1. **File → Open Folder** → `C:\app\dgpcorp` (thư mục **root** repo, có file `CMakeLists.txt` gốc).
